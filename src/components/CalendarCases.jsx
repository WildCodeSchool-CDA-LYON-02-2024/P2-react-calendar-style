...
export default function CalendarCases() {
  const [mode, setMode] = useState("mois"); // 'mois' par défaut
  const [selectedMonth, setSelectedMonth] = useState(null);

  const yearFr = {
    mois: [
      { name: "Janvier", nbDays: 31 },
      { name: "Février", nbDays: 29 }, // Année bissextile
      { name: "Mars", nbDays: 31 },
  ...
  const handleMonthClick = (index) => {
    setSelectedMonth(yearFr.mois[index]);
    setMode("nbDays");
  };
  ...
  return (
    <div className="calendarCasesContainer">
      <div>
        <button onClick={() => setMode("mois")}>Afficher les mois</button>
      </div>
      {mode === "mois" && (
        <div className="allMonthsContainer">
          {yearFr.mois.map((mois, index) => (
